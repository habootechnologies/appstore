...
class Provider(models.Model):
    ...
    facebook_page_token = fields.Char("Facebook Access Token")


    def text_to_url_text(self, message):
        ...
        for a_tag in soup.find_all('a'):
            ...
            # Check if the href attribute exists and isn't empty
            if url:
                # Replace the anchor tag with its URL
                # Note: This modifies the soup object. To get the final string, you can iterate through the modified soup.
                a_tag.replace_with(url)

        # Convert the soup object back to a string and print it
    ...
    def _onchange_graph_api_url(self):
        if self.meta_platform == 'instagram':
            self.graph_api_url = "https://graph.instagram.com/v24.0/"
        ...
        if self.graph_api_url and self.graph_api_token:
            ...
            if not page_content.get("error"):
                if self.meta_platform == 'instagram':
                    ...
                    url = "%s%s?fields=id,name,access_token,username,instagram_business_account,followers_count&access_token=%s" % (
                        self.graph_api_url,
                        page_content["id"],
                        self.graph_api_token,
                    )
                ...
                if content.get('access_token'):
                    self.facebook_page_token = content.get('access_token')
                    self.facebook_page_id = page_content["id"]                    
    ...
    def graph_api_messenger_send_message(self, recipient, message, quotedMsgId):
        if self.graph_api_authenticated:
            message = self.text_to_url_text(message)
            payload = json.dumps(
                {
    ...
    def messenger_send_media(self, attachment_id, recipient, sent_type):
        # print("messenger_send_media============", attachment_id, recipient, sent_type)
        t = type(self)
        # print("t============", t)
        fn = getattr(t, f"{self.provider}_messenger_send_media", None)
        # print("fn============", fn)
        res = fn(self, attachment_id, recipient, sent_type)
        # print("res============", res)
        return res

    def graph_api_messenger_send_media(self, attachment_id, recipient, sent_type):
        # print("graph_api_messenger_send_media====================", recipient, sent_type, attachment_id)
        if self.graph_api_authenticated:
    ...
    def instagram_send_media(self, attachment_id, recipient, sent_type):
        # print("instagram_send_media==============", )
        t = type(self)
        # print("t===========", t)
    ...
    def graph_api_instagram_send_media(self, attachment_id, recipient, sent_type):
        # print("graph_api_instagram_send_media====================", recipient, sent_type, attachment_id)
        if self.graph_api_authenticated:
            url = self.graph_api_url + self.account_id + "/messages"
            # print("url=====graph_api_instagram_send_media========", url)
            data = {
                "recipient": {
                    "id": recipient.instagram_account_id
                },
                "message": {
                    "attachment": {
                        "type": sent_type,
            ...
            # print("data=============", data)
            # "attachment": {"type": sent_type, "payload": {"attachment_id": attachment_id}}
            ...
            try:
                answer = requests.post(url, headers=headers, data=payload)
                # print("answer=========graph_api_instagram_send_media==========", answer.json())
            except requests.exceptions.ConnectionError:
                raise UserError("please check your internet connection.")
            if answer.status_code != 200:
                if json.loads(answer.text) and "error" in json.loads(answer.text):
    ...
    def _get_instagram_messenger_channel(self, partner, user, platform=False):
        ...
        if provider_channel_id:
            ...
            channel |= self.env["mail.channel"].sudo().create(channel_vals)
