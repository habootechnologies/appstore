# Migration Notes — Odoo 16.0

## Status

**Code-migrated only. NOT install-verified on a live Odoo 16.0 server.**

These modules were originally developed and tested on **Odoo 19.0**. The code
on this branch has been adapted for Odoo 16.0 via static analysis and known
API changes, but has **not** been installed and exercised on a running Odoo 16.0
instance.

## What was adapted

- Manifest versions: updated from 19.0.x.x.x to 16.0.x.x.x
- View XML syntax is unchanged: `<list>` and inline `invisible=` expressions were
  **introduced** in Odoo 16.0, so no view-level changes are needed
...
- Access rights (ir.model.access) are unchanged — verify they match your 16.0 security model
...
- [ ] Install each module on a clean Odoo 16.0 database
