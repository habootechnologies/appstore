# Migration Notes — Odoo 18.0

## Status

**Code-migrated only. NOT install-verified on a live Odoo 18.0 server.**

These modules were originally developed and tested on **Odoo 19.0**. The code
on this branch has been adapted for Odoo 18.0 via static analysis and known
API changes, but has **not** been installed and exercised on a running Odoo 18.0
instance.

## What was adapted

- Manifest versions: updated from 19.0.x.x.x to 18.0.x.x.x
- No view or Python API changes were required between Odoo 17 and 18 for the APIs
  used by these modules (`fields.*`, standard ORM, `http.route`, `ir.config_parameter`)
...
- Access rights (ir.model.access) are unchanged — verify they match your 18.0 security model
...
- [ ] Install each module on a clean Odoo 18.0 database
