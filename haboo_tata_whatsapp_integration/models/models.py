...
class TataWhatsappSession(models.Model):
    _name = 'tata.whatsapp.session'
    ...
    # session_messages = fields.One2many('tata.whatsapp.messages', 'session_id', string='Session Messages', copy=False)
    ...
    # def _compute_new_message_tigger(self):
    #     for rec in self:
    #         if rec.new_opporunity:
    #             print("--------", 333333333333,"----3333333333333333333---\n")
    #             rec.new_send_whatsapp_message()
    #             print("--------", 12345678765434567,"----12345678765434567---\n")
    def new_send_whatsapp_message(self):
        url = "https://wb.omni.tatatelebusiness.com/whatsapp-cloud/messages"
        access_token = self.env['ir.config_parameter'].sudo().get_param('haboo_tata_whatsapp_integration.session_token')
        if not access_token:
        ...
        # phone_no = '91' + self.customer_number
        data = {
            "to": self.customer_number,
            "type": "template",
            "source": "external",
            "template": {
                "name": "retail_product_inquiry",
                "language": {
                    "code": "en"
                },
                "components": [
                    {
                        "type": "body",
                        "parameters": [
                            {"type": "text", "text": self.agent_user_id.name},
                        ]
                    }
                ]
            }
    ...
    def new_send_whatsapp_message_shopify(self):
        url = "https://wb.omni.tatatelebusiness.com/whatsapp-cloud/messages"
        access_token = self.env['ir.config_parameter'].sudo().get_param('haboo_tata_whatsapp_integration.session_token')
        if not access_token:
            raise ValidationError("Enter Session token first")
        ...
        print("--------", self.agent_user_id.name,"----self.agent_user_id.name---\n")
        # phone_no = '91' + self.customer_number
        data = {
            "to": self.customer_number,
            "type": "template",
    ...
    def create_opportunity_wizard(self):
        ...
        return {
            ...
            'res_model': 'tata.opportunity.convert.wizard',
            'view_mode': 'form',
            'context': {
                'default_whatsapp_session_id': self.id,
    ...
    def action_view_opportunity(self):
        return {
            ...
            'res_model': 'crm.lead',
            'view_mode': 'list,form',
            'domain': [('whatsapp_session_id', '=', self.id)],
        }
    ...
    def _prepare_send_image(self):
        ...
        session_token = self.env['ir.config_parameter'].sudo().get_param('haboo_tata_whatsapp_integration.session_token')
        headers = {
            "accept": "application/json",
        ...
        if response.status_code in [200, 201]: 
            ...
            if mime_type.startswith("image/"):
                ...
                print("----------",222222222222,"----'222222222222'-------")
    ...
    def action_send_image(self, image_id):
        token = self.env['ir.config_parameter'].sudo().get_param('haboo_tata_whatsapp_integration.session_token')
        url = "https://wb.omni.tatatelebusiness.com/whatsapp-cloud/messages"
        access_token = token
        if not access_token:
            raise ValidationError("Enter Session token first")
...
class TataWhatsappMessages(models.Model):
    _name = 'tata.whatsapp.messages'
    _description = 'Tata Whatsapp Messages'

    session_id = fields.Many2one('tata.whatsapp.session', string='Session')
