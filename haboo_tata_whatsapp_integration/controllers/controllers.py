...
class TataWhatsappWebhook(http.Controller):
    ...
    def send_dropimage_whatsup(self, filename, file_data, mimetype, thread):
        session_token = request.env['ir.config_parameter'].sudo().get_param('haboo_tata_whatsapp_integration.session_token')
    ...
    @http.route('/send/wp/message', type='json', auth='user', methods=['POST'], csrf=False)
    ...
    def controller_send_message(self,message_body, res_id):
        ...
        access_token = request.env['ir.config_parameter'].sudo().get_param('haboo_tata_whatsapp_integration.session_token')
        current_id = request.env['tata.whatsapp.session'].sudo().search([('id','=',res_id)],limit=1)
        if not access_token:
    ...
    def controller_action_send_image(self, image_id, res_id):
        token = request.env['ir.config_parameter'].sudo().get_param('haboo_tata_whatsapp_integration.session_token')
        url = "https://wb.omni.tatatelebusiness.com/whatsapp-cloud/messages"
        current_id = request.env['tata.whatsapp.session'].sudo().search([('id','=',res_id)],limit=1)
        access_token = token
        if not access_token:
    ...
    def find_correct_user_id(self):
        next_user_id = False
        # latest_session = request.env['tata.whatsapp.session'].sudo().search(
        #     [('state', '=', 'new')], 
        ...
        user_session = request.env['whatup.user.session'].sudo().search([], order='id ASC')
        all_user_ids = user_session.mapped('user_ids.id')
        if not all_user_ids:
            return False  # Return None if no user IDs found
        latest_session = request.env['tata.whatsapp.session'].sudo().search(
        ...
        if not latest_session or not latest_session.agent_user_id:
            return False  # Return None instead of a Response object

        last_user_id = latest_session.agent_user_id.id  
        # if max(all_user_ids) > last_user_id:
        #     max
        #     ast_index = all_user_ids.index(last_user_id)
        ...
        return next_user_id  # Return only the ID, not a Response object

    @http.route('/tata/whatsapp/test', type='http', auth='public', methods=['POST'], csrf=False)
    def tata_whatsapp_messages_test_webhook(self, **kw):
        ...
        existing_session = request.env['tata.whatsapp.session'].sudo().search([], limit=1)
        
        message_rec = request.env['mail.message'].sudo().create({
            "message_type": 'comment', 
            'model': 'tata.whatsapp.session', 
        ...
        # return Response(json.dumps({"success": True, "message": "Processed successfully"}), content_type="application/json", status=200)
    ...
    @http.route('/tata/whatsapp/messages/json', type='json', auth='public', methods=['POST'], csrf=False)
    def tata_whatsapp_messages_webhook(self, **kw):
        data = json.loads(request.httprequest.data.decode("utf-8"))


    @http.route('/tata/whatsapp/messages', type='http', auth='public', methods=['POST'], csrf=False)
    def tata_whatsapp_messages_webhook(self, **kw):
        data = json.loads(request.httprequest.data.decode("utf-8"))
        print(data)
        channel = "whatsapp_channel"
        ...
        user_id = self.find_correct_user_id()
        today = date.today();
        existing_session = request.env['tata.whatsapp.session'].sudo().search([('customer_number', '=', data['contacts'][0]['wa_id']), ('last_massage_datetime', '>', fields.Datetime.now() - timedelta(days=30))], limit=1)
        message = {
            "data": data,
            "channel": channel
        }
        ...
        if existing_session:
            ...
            if attachment_data:
                ...
                message_rec = request.env['mail.message'].sudo().create({"attachment_ids": attachments.ids, "author_id": partner.id, "message_type": 'comment', 'model': 'tata.whatsapp.session', 'res_id': existing_session.id, "body": '-'})

            else:
                message_rec = request.env['mail.message'].sudo().create({"author_id": partner.id, "message_type": 'comment', 'model': 'tata.whatsapp.session', 'res_id': existing_session.id, "body": message_text})
        else:
            partner = request.env['res.partner'].sudo().create({
                'name': data['contacts'][0]['profile']['name'],
                'phone': data['contacts'][0]['wa_id'],
            })
            new_session = request.env['tata.whatsapp.session'].sudo().create({
                'session_id': data['id'],
            ...
            if attachment_data:
                ...
                attachments = attachments.sudo().create({
                        ...
                        'type': 'binary',
                        'mimetype': attachment_data['mime_type']})
                message_rec = request.env['mail.message'].sudo().create({"attachment_ids": attachments.ids, "author_id": partner.id, "message_type": 'comment', 'model': 'tata.whatsapp.session', 'res_id': new_session.id, "body": '-'})
            else:
                message_rec = request.env['mail.message'].sudo().create({"author_id": partner.id, "message_type": 'comment', 'model': 'tata.whatsapp.session', 'res_id': new_session.id, "body": message_text})
...
class TataWhatsappWebhookDLR(http.Controller):

    @http.route('/tata/whatsapp/messages/dlr', type='http', auth='public', methods=['POST'], csrf=False)
